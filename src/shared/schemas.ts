...
export const InspectionReportInputSchema = z.object({
  ...
  phone: z.string().min(1, 'Phone is required'),
  email: z.string().email('Invalid email'),
  agritectumContact: z.string().min(1, 'Company contact is required'),
  agritectumPhone: z.string().min(1, 'Company phone is required'),
  agritectumEmail: z.string().email('Invalid company email'),
